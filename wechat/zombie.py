...
with mss.mss() as sct:
    ...
    while True:
        ...
        cv2.circle(scale_img, (pos_pause[0], pos_pause[1]), radius=3, color=(0, 255, 0), thickness=2)

        # 确定当前的状态
        if cnt % 100 == 0:
            if(template_match(template_组队招募)):
                status="组队招募"
            elif(template_match(template_寰球救援)):
                status="寰球救援"
        ...
        if(status=="战斗"):
            ...
            for i in range(len(pts)):
                ...
                #logging.info(str(pt)+ str(pts[i]))
                #cv2.circle(scale_img, (pos_组队招募_sample[i][0], pos_组队招募_sample[i][1]), radius=3, color=(0, 0, 255), thickness=2)
            ...
            logging.info("level:"+str(cur_level))
            time.sleep(2.0)
            status="战斗中"
        elif(status=="战斗中"):
            if(cur_level >= 10):
                click_once(pos_pause[0], pos_pause[1])
                time.sleep(0.3)
                click_once(pos_exit[0], pos_exit[1])
                status="未知"
        else:
            a = 0
